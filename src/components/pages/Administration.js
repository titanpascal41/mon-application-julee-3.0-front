...
const Administration = ({ activeSubPage: activeSubPageProp }) => {
  ...
  const nbUtilisateursAffectes = profilToToggle
    ? utilisateurs.filter((u) => u.profilId === profilToToggle.id).length
    : 0;

  const confirmDeactivation = async () => {
    if (!motifDesactivation.trim()) return;
    const resultat = await toggleActivationProfil(
      profilToToggle.id,
      false,
    ...
    if (resultat.succes) {
      setMessage({
        type: "success",
        text: `Profil "${profilToToggle.nom}" désactivé.`,
      });
      await chargerLesProfils();
      setTimeout(() => setMessage({ type: "", text: "" }), 3000);
    } else {
      setMessage({
        type: "error",
        text: resultat.message || "Erreur lors de la désactivation",
      });
    }
  ...
  const confirmReactivation = async (profil) => {
    ...
    if (resultat.succes) {
      ...
      setMessage({
        type: "error",
        text: resultat.message || "Erreur lors de la réactivation",
      });
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    if (editingProfil) {
      resultat = await mettreAJourProfil(
        editingProfil.id,
        formData.nom,
        formData.code,
      );
  ...
  const allPermissionsChecked =
    ...
    permissionsData.every(
      (p) => p.access && p.créer && p.lire && p.modifier && p.supprimer,
    );
  ...
  */
  ...
  const getPasswordStrength = (mdp) => {
    ...
    if (score <= 2)
      return { niveau: "Faible", couleur: "#ef4444", largeur: "33%" };
    if (score <= 3)
      return { niveau: "Moyen", couleur: "#f59e0b", largeur: "66%" };
  ...
  const subPages = {
    profils: {
      ...
      description:
        "Créez et configurez les profils d'accès et leurs permissions.",
        ...
        <div>
                ...
                >
                  {!showPermissionsInMainForm ? (
                    ...
                    <>
                      ...
                      >
                        <label htmlFor="code">Code (acronyme)</label>
              ...
              <table className="data-table">
                <thead>
                  <tr>
                    ...
                    {peutVoirActionsProfils && <th>Actions</th>}
                  </tr>
                </thead>
                ...
                <tbody>
                  {profilsFiltres.slice((pageProfils - 1) * ITEMS_PER_PAGE, pageProfils * ITEMS_PER_PAGE).map((profil) => (
                      <tr
                        key={profil.id}
                        style={{
                          opacity: profil.actif === false ? 0.6 : 1,
                          backgroundColor:
                            profil.actif === false ? "#F3F4F6" : "inherit",
                        }}
                      >
                        <td>
                          {profil.code ? (
          ...
          {showProfilOccupeModal && (
            <div
              ...
              onClick={() => setShowProfilOccupeModal(false)}
            >
              <div
                className="modal-content"
                style={{ maxWidth: "420px", width: "90%" }}
              ...
              >
                ...
                <div style={{ padding: "24px 32px" }}>
                  ...
                  >
                    ...
                    >
                      Le profil <strong>"{profilOccupeName}"</strong> est
                      actuellement assigné à un ou plusieurs utilisateurs.
                      <br />
                      Il ne peut pas être modifié tant que des utilisateurs lui
                      sont rattachés.
                    </p>
                  </div>
                </div>
          ...
          {showDeactivateModal && (
            ...
            >
              ...
              >
                ...
                <div style={{ padding: "24px 32px" }}>
                  <p style={{ fontSize: "14px", color: "#4B5563", marginBottom: nbUtilisateursAffectes > 0 ? "12px" : "20px" }}>
                    Vous allez désactiver le profil{" "}
                    <strong>"{profilToToggle?.nom}"</strong>.<br />
                    Veuillez indiquer le motif de désactivation.
                  </p>
                  {nbUtilisateursAffectes > 0 && (
                    <div style={{
                      ...
                      <i className="fa-solid fa-triangle-exclamation" style={{ color: "#F97316", marginTop: "2px", flexShrink: 0 }}></i>
                      <p style={{ margin: 0, fontSize: "13px", color: "#92400E" }}>
                        <strong>{nbUtilisateursAffectes} utilisateur{nbUtilisateursAffectes > 1 ? "s" : ""}</strong> {nbUtilisateursAffectes > 1 ? "sont assignés" : "est assigné"} à ce profil.<br />
                        {nbUtilisateursAffectes > 1 ? "Ils ne pourront" : "Il ne pourra"} plus se connecter après la désactivation.
                      </p>
                    </div>
                  )}
                  <label
                ...
                >
                  ...
                  <button
                    ...
                    style={{
                      padding: "8px 20px",
                      backgroundColor: motifDesactivation.trim()
                        ? "#F59E0B"
                        : "#D1D5DB",
                      ...
                      fontSize: "14px",
                      fontWeight: "600",
                      cursor: motifDesactivation.trim()
                        ? "pointer"
                        : "not-allowed",
                    }}
                  >
                  ...
                  <div className="form-group">
                    ...
                    >
                      ...
                      >
                        <i
                          className={
                            showUserPassword ? "fas fa-eye-slash" : "fas fa-eye"
                          }
                        ></i>
                      </button>
                    </div>
                    ...
                    {userFormData.motDePasse &&
                      (() => {
                        const force = getPasswordStrength(
                          userFormData.motDePasse,
                        );
                        return force ? (
                          <div style={{ marginTop: "6px" }}>
                            ...
                            >
                              — 8 car. min · 1 majuscule · 1 chiffre · 1
                              caractère spécial
                            </span>
                          </div>
                        ) : null;
              ...
              <table className="data-table">
                <thead>
                  <tr>
                    ...
                    {peutVoirActionsUtilisateurs && <th>Actions</th>}
                  </tr>
                </thead>
  ...
  return (
    <div className="page-container">
      ...
      {showViewPermissionsModal && (
        <div
          className="modal-overlay"
          onClick={handleCloseViewPermissionsModal}
        >
          <div
            className="modal-content"
            onClick={(e) => e.stopPropagation()}
                          ...
                          <div>
                            {perms.map((perm, i) => (
                              ...
                              >
                                ...
                                >
                                  <span style={badgeStyle(perm.lire)}>
                                    Lire
                                  </span>
                                  <span style={badgeStyle(perm.créer)}>
                                    Créer
                                  </span>
                                  <span style={badgeStyle(perm.modifier)}>
                                    Modifier
                                  </span>
                                  <span style={badgeStyle(perm.supprimer)}>
                                    Supprimer
                                  </span>
                                </div>
                              </div>
